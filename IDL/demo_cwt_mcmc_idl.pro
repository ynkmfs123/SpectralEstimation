...
PRO DEMO_CWT_MCMC_PLOT_RESULTS_FINAL, time_sec, signal, has_real_time, $
                                      ...
                                      SAVE_PNG=save_png, OUT_PNG=out_png, CWT_CT=cwt_ct
  COMPILE_OPT IDL2
  ...
  IF KEYWORD_SET(save_png) THEN BEGIN
    IF N_ELEMENTS(out_png) EQ 0 THEN out_png = 'cwt_mcmc_result.png'
    TVLCT, rr, gg, bb, /GET
    img = TVRD()
    WRITE_PNG, out_png, img, rr, gg, bb
    PRINT, 'Saved figure to: ', out_png
  ENDIF
END

PRO DEMO_CWT_MCMC_IDL_FINAL, SAVE_PNG=save_png, CWT_CT=cwt_ct
  COMPILE_OPT IDL2
  ...
  DEMO_CWT_MCMC_PLOT_RESULTS_FINAL, time_sec, signal, has_real_time, $
                                    periods, power, coi, sig_mask, $
                                    SAVE_PNG=save_png, OUT_PNG='cwt_mcmc_result_final.png', $
                                    CWT_CT=cwt_ct
END

PRO DEMO_CWT_MCMC_IDL, SAVE_PNG=save_png, CWT_CT=cwt_ct
  COMPILE_OPT IDL2

  DEMO_CWT_MCMC_IDL_FINAL, SAVE_PNG=save_png, CWT_CT=cwt_ct
END
