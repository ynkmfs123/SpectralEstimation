...
FUNCTION CWT_MCMC_FFTFREQ_ANGULAR, n, dt
  ...
  FOR k = 0L, n - 1L DO BEGIN
    IF k LE posmax THEN kk = DOUBLE(k) ELSE kk = DOUBLE(k - n)
    freqs[k] = 2D * !DPI * kk / (DOUBLE(n) * DOUBLE(dt))
  ENDFOR
...
FUNCTION CWT_MCMC_LOGLIKE_T, th_t, y_t, logf
  ...
  m = EXP(th_t[0]) * EXP(th_t[1] * DOUBLE(logf)) + EXP(th_t[2])
...
FUNCTION CWT_MCMC_MCMC, y, freqs, n_iter, burn_frac, thin, seed, $
  ...
  burn = LONG(DOUBLE(n_iter) * DOUBLE(burn_frac))
  keep_count = 0L
  theta_sum = DBLARR(3L, t_len)
  sigmas_sum = DBLARR(3L)

  FOR it = 0L, LONG(n_iter) - 1L DO BEGIN
    FOR i = 0L, t_len - 1L DO BEGIN

      prop = theta[*, i]
      prop[0] += RANDOMN(rng_seed, /DOUBLE) * DOUBLE(prop_sig_loga)
      IF prop[1] LT 0D THEN BEGIN
        ...
        IF ALOG(RANDOMU(rng_seed, /DOUBLE)) LT (dll + dlp) THEN BEGIN
          theta[*, i] = prop
          cur_ll[i] += dll
        ENDIF
      ENDIF
      ...
      prop = theta[*, i]
      prop[1] += RANDOMN(rng_seed, /DOUBLE) * DOUBLE(prop_sig_alpha)
      IF prop[1] LT 0D THEN BEGIN
        dll = CWT_MCMC_LOGLIKE_T(prop, y[*, i], logf) - cur_ll[i]
        dlp = CWT_MCMC_LOGPRIOR_DELTA_THETA(theta, i, prop, sigmas, mu_anchor, sd_anchor)
        IF ALOG(RANDOMU(rng_seed, /DOUBLE)) LT (dll + dlp) THEN BEGIN
      ...
      prop = theta[*, i]
      prop[2] += RANDOMN(rng_seed, /DOUBLE) * DOUBLE(prop_sig_logc)
      IF prop[1] LT 0D THEN BEGIN
        dll = CWT_MCMC_LOGLIKE_T(prop, y[*, i], logf) - cur_ll[i]
        dlp = CWT_MCMC_LOGPRIOR_DELTA_THETA(theta, i, prop, sigmas, mu_anchor, sd_anchor)
        IF ALOG(RANDOMU(rng_seed, /DOUBLE)) LT (dll + dlp) THEN BEGIN
    ...
    FOR jj = 0L, 2L DO BEGIN
      ls_prop = ALOG(sigmas[jj]) + RANDOMN(rng_seed, /DOUBLE) * DOUBLE(prop_sig_logsig)
      sig_prop = EXP(ls_prop)

      lp_rw2_prop = CWT_MCMC_RW2_LOGPRIOR_FULL(theta[jj, *], sig_prop)
      lp_rw2_curr = CWT_MCMC_RW2_LOGPRIOR_FULL(theta[jj, *], sigmas[jj])
      lp_sig_prop = CWT_MCMC_HALF_T_LOGPRIOR_SIGMA(sig_prop, DOUBLE(nu_half_t), taus[jj])
      lp_sig_curr = CWT_MCMC_HALF_T_LOGPRIOR_SIGMA(sigmas[jj], DOUBLE(nu_half_t), taus[jj])

      lp_prop_total = lp_rw2_prop + lp_sig_prop + ls_prop
      lp_curr_total = lp_rw2_curr + lp_sig_curr + ALOG(sigmas[jj])

      IF ALOG(RANDOMU(rng_seed, /DOUBLE)) LT (lp_prop_total - lp_curr_total) THEN sigmas[jj] = sig_prop
    ENDFOR

    IF (it GE burn) AND (((it - burn) MOD LONG(thin)) EQ 0L) THEN BEGIN
      theta_sum += theta
      sigmas_sum += sigmas
      keep_count += 1L
    ENDIF
  ...
  denom = DOUBLE(keep_count > 1L)
  theta_mean = TRANSPOSE(theta_sum / denom)
  sigmas_mean = sigmas_sum / denom

  bg_spectra = DBLARR(n_freq, t_len)
  FOR i = 0L, t_len - 1L DO BEGIN
    loga_mean = theta_mean[i, 0]
    alpha_mean = theta_mean[i, 1]
    logc_mean = theta_mean[i, 2]
    bg_spectra[*, i] = EXP(loga_mean) * (DOUBLE(freqs)^alpha_mean) + EXP(logc_mean)
  ENDFOR
